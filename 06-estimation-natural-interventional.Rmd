# Construct a robust semiparametric efficient estimator: the case of the natural direct effect
...
- Here we show the detaild of how to construct an estimator for the
  NDE for illustration, but the construction of this estimator is a
  bit involved and may be complex in daily research practice
- For practice, we will teach you how to use our packages _medoutcon_
  and _medshift_ for automatic implementation of these estimators of
  the NDE and other parameters
...
\begin{align*}
    \hat\psi &= \frac{1}{n}\sum_{i=1}^n\color{RoyalBlue}{\bigg\{ \frac{I(A_i=1)}{\hat\P(A_i=1\mid W_i)}\frac{\hat\P(M_i\mid A_i=0,W)_i}{\hat\P(M_i\mid A_i=1,W_i)} -
      \frac{I(A=0)}{\hat\P(A_i=0\mid W_i)}\bigg\}} \color{Goldenrod}{[Y_i-\hat\E(Y\mid A_i,M_i,W_i)]}  \\
    &+ \frac{1}{n}\sum_{i=1}^n\color{RoyalBlue}{\frac{I(A=0)}{\P(A=0\mid
	W)}}\color{Goldenrod}{\big\{\hat Q(M_i,W_i) - \hat\E[\hat Q(M_i,W_i) \mid W_i,A_i=0] \big\}}\\
    &+ \frac{1}{n}\sum_{i=1}^n\color{Goldenrod}{\hat\E[\hat Q(M_i,W_i) | W_i,A_i=0]}
\end{align*}
...
Thus we can change the expression of the estimator a bit as follows. First, some more
...
Recall that the semiarametric efficient estimator can be computed  in the following steps:
